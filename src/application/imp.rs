...
use crate::config::{REPO, TEMPLATE_DIR};
...
fn folder_project_name() -> Option<String> {
    ...
    match folder_list {
        Ok(child) => {
            let output = child
                .wait_with_output()
                .expect("Folder_project_name error!")
                .stdout;
            ...
            let mut item_info_list: Vec<String> = list
                .get(1)
                .map(|item_list| item_list.split(" ").map(|item_info| item_info.to_string()))
                .unwrap()
                .collect();
            item_info_list.pop() //take project name
        }
        _ => None,
    }
}
